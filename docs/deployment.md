...
| `/app/bugfix-allowlist.json` 等四份授权名单 | 对应同名文件（只读） | 见 [configuration.md §2.2/2.3](configuration.md#22-代码修改授权部门为主--open_id-兜底强制校验) |
...
2. **Codex CLI 鉴权（仅 `CLI_PROVIDER=codex` 时需要）**：镜像里也装了 `codex`。无头环境官方推荐 **`CODEX_API_KEY` 环境变量**（API 计费），写进 `.env` 即可被子进程继承；备选：把已登录机器上的 `~/.codex/auth.json`（ChatGPT 订阅额度）挂载到容器内 `/home/appuser/.codex/auth.json`——该文件等同密码，注意权限。
