...
| `/app/IDENTITY.md` | `./IDENTITY.md`（只读） | 助手身份（名字/描述）；镜像内已烤入默认，挂载可覆盖，`config-ui` 可编辑。**不挂载也能启动**（用镜像内默认） |
...
2. **Codex CLI 鉴权（仅 `CLI_PROVIDER=codex` 时需要）**：镜像里也装了 `codex`，按计费方式二选一：
   ...
   - **容器内必须关掉 codex 自带沙箱**：`docker-compose.yml` 已为 `feishubot` 服务设 `CODEX_UNSANDBOXED=true`。codex 默认用 bwrap 建沙箱，但 Docker 容器内 bwrap 无法为沙箱创建 network namespace 配 loopback，会报 `bwrap: loopback: Failed RTM_NEWADDR: Operation not permitted`，导致连 `pwd`/`rg` 都跑不起来。关掉后由容器隔离 + 注册表路径白名单 + 触发人授权名单兜底安全（见 [configuration.md](configuration.md) `CODEX_UNSANDBOXED` 与 [handlers.md](handlers.md) §6）。**无需**给容器加 `privileged`/`cap_add`/`security_opt` 去迁就 bwrap。
...
| codex 任务报 `bwrap: ... Operation not permitted`（如 `RTM_NEWADDR` / `create a new namespace`） | codex 自带 bwrap 沙箱在容器内跑不起来。确认 `feishubot` 服务设了 `CODEX_UNSANDBOXED=true`（compose 已内置），别再靠 `privileged`/`cap_add` 迁就 bwrap。见 §4 |
